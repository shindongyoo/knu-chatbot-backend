...
async def stream_answer(req: QuestionRequest):
    ...
    def event_generator():
        try:
            ...
            2. **[유연한 답변 규칙]** 만약 사용자의 질문('A')이 "취업 정보"처럼 너무 광범위한데, 검색된 자료('B')가 "[삼성전자] 채용 공고"처럼 매우 구체적인 내용일 경우가 있습니다.

            3. 이럴 때, "정보를 찾을 수 없다"고 바로 거절하지 마세요. 대신, **"질문하신 'A'에 대한 포괄적인 정보는 찾지 못했지만, 관련 자료로 'B'를 찾았습니다."** 라고 말한 뒤, **당신이 찾은 자료 'B'의 핵심 내용을 요약해서** 알려주세요.

            4. 만약 검색된 자료('B')가 사용자의 질문('A')과 **전혀 관련이 없거나 내용이 비어있을 경우에만** "죄송합니다, 제공된 자료에서 해당 정보를 찾을 수 없습니다."라고 답변하세요.
